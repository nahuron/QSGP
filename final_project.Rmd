...
```{r get shapefiles for species, cache=TRUE}
#list of shape files for each species to cut enviro data to
leiodist <- list.files(path = "/Volumes/GoogleDrive/My Drive/Global_Trait/Leiocephalus_Project/data/distribution/Caribbean_Herp_Data/polygons/Reptiles/", pattern = "^Leiocephalus.*\\.shp$", full.names = TRUE)
...
for(a in seq_along(countries)){
  all_countries[[a]] <- getData('GADM', country = countries[a], level = 0)
}
...
plot(crop(comb_carib,readOGR(leiodist[1])))

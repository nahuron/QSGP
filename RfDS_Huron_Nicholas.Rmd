...
this code does not work, because the i is dotted in the assignment and not dotted in the call for the variable to see what it contains
  ...
  2. tweak each of the commands so that they work
...
We are now using the `nycflights13` package and dataset `flights`
...
```
  ...
  - arrive > 120 min late but did not depart late
```{r}
filter(flights, arr_delay > 120 & dep_delay <= 0) #29
```

  - delayed >= 60 min and made up over 30 min in flight (difference in initial and final delays >30)
```{r}
filter(flights, dep_delay >= 60 & (dep_delay - arr_delay) > 30) #1844
```

  - departed between 00:00 and 06:00 inclusive ( dep_time >= 0000 and dep_time <=0600)
```{r}
...
```
  ...
  3. how many flights are missing dep_time? what other vars are missing in these? what might these rows represent?
```{r}
...
###Exercises 5.3.1
  ...
  1. 
```{r}
arrange(flights, desc(rowSums(is.na(flights))))
#thank you stackoverflow, this sorts by the number of rows with NA values for each column
```
...
###Exercises 5.5.2
  ...
  1. convert dep_time and sched_dep_time from numeric representation of time to min since midnight
```{r}
transmute(flights,
          dep_time,
...
```
  ...
  2. expect to see a difference between air_time and the difference in arr and dep times, since they are not in minutes and air_time is. Furthermore, they may include different time zones, which leads to incorrect  calculations
```{r}
...
still not matching up for all of these, you can see from the first entry that this is because of not accounting for changing of dates
  ...
  4. find 10 most delayed flights via ranking function, use row_number() to skip ties
```{r}
...
This depends on the question. Since there are some flights with wonky arrival delay data, I may stick with departure. Additionally, departure may be a decent indicator of how smoothly everything is running, assuming expected stable/consistent flight times.

  2. find alternative to code without using `count()`
```{r}
...
```
  ...
  3. the definition: `(is.na(dep_delay) | is.na(arr_delay) )`  for canceled flights is suboptimal. why? Which col is more important?
...
if we take the original function above and remove the `!` from the `arr_delay` query, we can see that there are a number of flights that contain a departure delay but have NA for the arrival delay but have an actual arrival time and scheduled arrival time. While they do not have air time values either, this suggests that some flights may have not been cancelled (perhaps redirected)? With this in mind, it seems like `!is.na(dep_delay)` is more important.
```{r}
not_cancelled <- flights %>% 
  filter(!is.na(dep_delay), is.na(arr_delay))
View(not_cancelled)
...
It seems like F9 has the worst delays on average for both arr and dep...
...
I should visit the vignette("window-functions") sometime

###Exercises 5.7.1

  1. mutate and filter "helpful" functions
...
cumsum, cumprod, cummin, cummax, cummeans    
again the function is applied by group
```{r}
(flights %>%
       group_by(carrier) %>%
...
>does not look like there is a crazy relationship, but it is positive...
  ...
  6. Look at destinations for flights that are suspiciously fast. Compute the air time a flight relative to the shortest flight to that destination. Which flights were most delayed in the air?
```{r}
flights %>%
...
**Filtering joins** select observations from one data frame based on whether or not they match an observation in another table.
...
You would need the origins, destinations, and flight number identifiers for each flight. Beyond that, you would likely want the gps data for each origin and destination. Since the airport identifiers are not consistent, you would need a universal key. To include all of these variables, you would need the `flights` and `airports` at a minimum.

  2. I forgot to draw the relationship between weather and airports. What is the relationship and how should it appear in the diagram?
  
These two data frames connect through `origin` and `faa`, which both keep the same three letter code for the airports that they reference (more specifically outlined in `airports`). Thus, they would show an arrow between the two variables that go through `origin` and probably `dest` in flights. `weather` would also connect to flights via additional variables.
...
You could create an extra table that provides names/identifiers for these dates, in addition to the dates themselves. You could use the year, month, and day as keys to join to the main flights dataset. If there are region specific "special" dates, you could format the table where each observation also contains an affected airport (thus you might have duplicates for a particular date).
...
You can verify primary keys with `count()`:
```{r}
planes %>% 
  count(tailnum) %>% 
  filter(n > 1)
...
Sometimes it can be difficult to find a primary key for a datatable:
```{r}
flights %>% 
...
In this case, `flights` does not have a primary key. We can give it one with `mutate()` and `row_number()`. This action is called creating a **surrogate key**.

###Exercises 13.3.1
  1. Add a surrogate key to flights.
```{r add surrogate to flights}
flights %>%
...
>I am not doing this one at the moment.

###Section 13.4
####Mutating Joins

Make smaller version of `flights` for demonstrating `left_join()`
```{r}
(flights2 <- flights %>% 
...
the base `mutate` function can be used to get the same effect:
```{r}
flights2 %>%
  select(-origin, -dest) %>% 
...
**inner join** (drops any rows that do not follow the key for joining):
```{r}
x %>% 
  inner_join(y, by = "key")
```
...
It is clear that the key is primary in y but not x (unique values)! However, you can get all combinations of values when neither has a primary key that is being joined:
```{r}
x <- tribble(
  ~key, ~val_x,
     1, "x1",
...
>The **natural join** uses all variables instead of specifying a `by =`
...
You can also join by specifying a single variable (`by = "tailnum"`) or by one variable set to equal another in the other dataframe (`by = c("a" = "b")`).
...
Calculate arrival delay without `NA` values and join by airport codes with `left_join()`. Then, use example plot with new `dest_delay` object. There are a few codes that are missing coords (n=4).
```{r}
(dest_delay <- flights %>%
  group_by(dest) %>%
...
dest_delay %>%
  group_by(avg_arr_delay) %>%
ggplot(aes(lon, lat)) +
    borders("state") +
    geom_point(mapping = aes(colour = avg_arr_delay, size = avg_arr_delay)) +
...
```{r}
print("We know that the latest year in flights is 2013, so give the planes ages!")
...
flights %>%
  ...
  group_by(age) %>%
  summarise(avg_arr_delay = mean(arr_delay, na.rm = T)) %>%
  ggplot(mapping = aes(x = age, y = avg_arr_delay)) +
  geom_line()
...
flight_weather %>%
  ...
  ggplot() +
    geom_line(mapping = aes(x = precip, y = avg_arr_delay))

flight_weather %>%
  group_by(wind_speed) %>%
  summarise(avg_arr_delay = mean(arr_delay, na.rm = TRUE)) %>%
  ggplot() +
    geom_line(mapping = aes(x = wind_speed, y = avg_arr_delay))

flight_weather %>%
  group_by(wind_gust) %>%
  summarise(avg_arr_delay = mean(arr_delay, na.rm = TRUE)) %>%
  ggplot() +
    geom_line(mapping = aes(x = wind_gust, y = avg_arr_delay))

flight_weather %>%
  group_by(visib) %>%
  summarise(avg_arr_delay = mean(arr_delay, na.rm = TRUE)) %>%
  ggplot() +
    geom_line(mapping = aes(x = visib, y = avg_arr_delay))
```

Precipitation seems to be the winner here...
...
`semi_join()` keeps all observations in x that are in y
`anti_join()` removes those same matches

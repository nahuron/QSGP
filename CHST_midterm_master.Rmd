...
##Abstract
TEXT NEEDS TO GO HERE (~50-100 word abstract that describes your findings on Pennsylvania’s Invasive Species Problem to a general audience.)
...
DR_invasives <- ggplot(data = Rep_aqua_invasives)+
  ...
  labs(y = "Latitude of PA in Degrees", x = "Longitude of PA in Degrees", title = "Invasive Species Richness By Federal Congressional District and Political Party")
...
Based on our findings of the species richness of our political parties map, we can deduct that Republican areas tend to have a higher species richness of invasive species. On the other hand, the Democratic areas represent lower species richness of invasive species.  There are many factors that contribute to this relationship. For example, the fact that as of the most recent presidential election, Pennsylvania is slightly more red than blue based on the election results. Given this result, the Republican areas tend to have the highest species richness of invasive species. This corresponds to the fact that it is a mostly rural area and therefore is an ideal environment for invasive species to spread. By definition an invasive species is a species that is usually non-native that becomes established in a new area and spreads without further human intervention beyond its point of initial release. In order for an invasive species to be termed an invasive species it must complete several stages. First, it has to be introduced to an area, then it must colonize, after which it then has to be established by reproducing and finally it has to spread. Once it spreads, it is now an invasive specie.   On the contrary, the lower species richness of invasive species corresponds to the Democratic areas, one of which is Philadelphia County which is an urban area and would mean that it is a  less suitable environment for invasive species to be introduced, colonize, establish themselves and ultimately spread.
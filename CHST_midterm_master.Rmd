...
First, we read in the `iMap_spp.csv` file to a new object `imap_spp`. We can trim the new object down to all entries we already tidied by selectively joining `imap_spp` to `invasives2`. The resulting object is `invasives3`. 
...
```{r}
#Authors: NGS
srPerCongressional <- over(invasives2, congress)
srPerCongressional <- cbind(srPerCongressional, invasives2$state_scientific_name)
colnames(srPerCongressional)[colnames(srPerCongressional) %in% 'invasives2$state_scientific_name'] <- 'species' 
